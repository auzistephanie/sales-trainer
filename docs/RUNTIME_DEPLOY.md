...
`DEEPSEEK_API_KEY` · `UPSTASH_REDIS_REST_URL` · `UPSTASH_REDIS_REST_TOKEN` · `TELEGRAM_BOT_TOKEN` · `TELEGRAM_CHAT_ID` · `GITHUB_TOKEN` · `GITHUB_REPO` · `JINA_API_KEY`（optional，抓取加速） · `FIRECRAWL_API_KEY`（optional，非 JobsDB 網站後備）· `SCRAPERAPI_KEY`（JobsDB 抓 JD 必需；冇就叫用戶貼 JD）· `GOOGLE_CREDENTIALS` + `GOOGLE_DRIVE_FOLDER_ID`（optional，CV 上 Drive）
